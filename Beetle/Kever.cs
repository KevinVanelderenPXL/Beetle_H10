...
public class Kever
{
    ...
    public void VisualizeBeetle(Canvas activeCanvas)
    {
        ...
        Canvas.SetLeft(beetleBody, X - BeetleSize / 2); //(object, x-pixels from leftside) -> represents the center of beetlebody instead of topleft corner coordinates
        Canvas.SetTop(beetleBody, Y - BeetleSize / 2);  //(object, y-pixels from topside)
    ...
    public void AutoMove(Canvas canvas)
    {
        ...
        Y += DY * StepSize;                 // example: X = 2 --> X = 2 + (1 * 0.2) --> new X-coordinate = 2.2
